...
| E5b | ~~MCP injection for opencode.~~ Resolved by the ``opencode run`` per-task rewrite: ``OpenCodeExecutor`` now spawns one ``opencode run`` subprocess per task and injects ``context.mcp_servers`` as the ``mcp`` block of an inline config passed via the ``OPENCODE_CONFIG_CONTENT`` env var. Each BSNexus-style ``{name: {url, headers}}`` entry becomes a ``{type: "remote", url, headers, enabled: true}`` opencode MCP server. | `worker/executors.py::OpenCodeExecutor.execute` | BSNexus / external user picks `opencode` executor and needs MCP | **Done (opencode run rewrite)** |
...
| E6b | ~~Per-session workspace_dir for opencode.~~ Resolved by the ``opencode run`` per-task rewrite: each task is its own ``opencode run --dir <workspace>`` subprocess (spawned with ``cwd=<workspace>``), so the workspace dir is honored per-task just like claude/codex. The shared long-lived ``opencode serve`` is gone. | `worker/executors.py::OpenCodeExecutor` | A BSNexus tenant picks ``executor_type=opencode`` and needs per-task workspace isolation | **Done (opencode run rewrite)** |
